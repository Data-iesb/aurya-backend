"""
RAG da Atena — busca semântica nos guias do IESB (Extensão Curricularizada e
Atividades Complementares) e nas apostilas da Athena Educacional (dados no
Postgres, schema aurya).
"""
...
CORPUS_TABLES = {
    "iesb": "iesb_rag_chunks",
    "educacional": "athena_educacional_chunks",
}
...
@lru_cache(maxsize=len(CORPUS_TABLES))
def _load(corpus: str) -> tuple:
    table = CORPUS_TABLES[corpus]
    engine = TrinoConnection.get_engine("iesb")
    with engine.connect() as conn:
        rows = conn.exec_driver_sql(
            f"SELECT source, page, section, text, embedding "
            f"FROM {SCHEMA}.{table} ORDER BY faiss_id"
        ).fetchall()
    ...
    print(f"[RAG:{corpus}] {len(metas)} chunks carregados do banco")
    return metas, vectors


def search(query: str, top_k: int = TOP_K, corpus: str = "iesb") -> str:
    """Retorna os trechos mais relevantes do corpus, com fonte e página."""
    metas, vectors = _load(corpus)
